...
public class IndicatorAdapterImpl implements IndicatorAdapter {
  ...
  public Indicator indicator(int index) {
    ...
    return Indicator.newBuilder().setName(name()).setValue(value).setParams(params).build();
  }
  ...
  private String name() {
    ImmutableList<?> indicatorId = firstNonNull(indicatorIdSupplier.get(), ImmutableList.of());
    ImmutableList<Object> parts =
        ImmutableList.builder().add(params.getTypeCase().name()).addAll(indicatorId).build();
